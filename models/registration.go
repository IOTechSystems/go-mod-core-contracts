...
func (reg Registration) Validate() (bool, error) {
	if !reg.isValidated {
		...
		if reg.ProcessFrequency == ProcessFrequencyNone {
			reg.ProcessFrequency = ProcessFrequencyZero
		}
		if _, err := time.ParseDuration(reg.ProcessFrequency); err != nil {
			return false, NewErrContractInvalid(fmt.Sprintf("ProcessFrequency invalid: %s", err.Error()))
		}
